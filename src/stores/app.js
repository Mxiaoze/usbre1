...
export const useAppStore = defineStore('app', () => {
  ...
  const openlistCoreStatus = ref({ running: false })
  const remoteConfigs = ref({})
  const mountInfos = ref([])
  const logs = ref([])
  const isCoreLoading = ref(false)
  const error = ref(undefined)
  const updateAvailable = ref(false)
  const updateCheck = ref(null)
  const openlistProcessId = ref(undefined)

  const defaultRcloneFormConfig = {
    name: '',
    type: 'webdav',
    url: '',
    vendor: '',
  ...
  async function tryCatch(fn, msg) {
    try {
      return await fn()
    } catch (e) {
      error.value = msg
  ...
  async function createRemoteConfig(name, type, config) {
    try {
      const fullConfig = {
        name,
        type,
  ...
  async function updateRemoteConfig(name, type, config) {
    try {
      const fullConfig = {
        name: config.name,
        type,
  ...
  async function deleteRemoteConfig(name) {
    try {
  ...
  async function mountRemote(name) {
    try {
      const config = settings.value.rclone.mount_config[name]
      if (!config) {
  ...
  async function unmountRemote(name) {
    try {
  ...
  async function safeUpdateTrayMenu(running) {
    try {
  ...
  function setUpdateAvailable(available, updateInfo) {
